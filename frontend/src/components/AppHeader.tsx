...
import { MoreVertical, X, Home, Sparkles, List, Clock, Trophy, ShieldCheck, CreditCard } from 'lucide-react';
...
const menuItems: { icon: React.ReactNode; label: string; screen: Screen; badge?: string }[] = [
  { icon: <Home className="w-4 h-4" />, label: 'Home', screen: { name: 'home' } },
  { icon: <Sparkles className="w-4 h-4" />, label: 'Practice Quiz', screen: { name: 'quiz', questions: [] } },
  { icon: <List className="w-4 h-4" />, label: 'Browse by Topic', screen: { name: 'topics' } },
  { icon: <Clock className="w-4 h-4" />, label: 'Browse by Year', screen: { name: 'topics' } },
  { icon: <Trophy className="w-4 h-4" />, label: 'Score History', screen: { name: 'history' } },
  {
...
export default function AppHeader({ onNavigate, currentScreen }: AppHeaderProps) {
    ...
    <header className="bg-primary text-primary-foreground shadow-md sticky top-0 z-40">
      <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
                ...
                {menuItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1 text-sm font-body font-medium">{item.label}</span>
                      {item.badge && (
                        <span
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] opacity-75 font-body">Previous Year Questions Practice</p>
        </div>
      </div>
    </header>
  );
