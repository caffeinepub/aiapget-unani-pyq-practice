...
export default function HistoryScreen({ onBack }: HistoryScreenProps) {
  ...
  const avgScore =
    history && history.length > 0
      ? Math.round(history.reduce((sum, a) => sum + (a.correct / a.total) * 100, 0) / history.length)
      : 0;
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="bg-primary text-primary-foreground shadow-md sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="font-heading font-bold text-base flex-1">Score History</h1>
          {history && history.length > 0 && (
            <button
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-5 space-y-4">
        ...
        {!isLoading && history && history.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: <Trophy className="w-4 h-4 text-gold" />, label: 'Attempts', value: history.length },
              { icon: <BarChart2 className="w-4 h-4 text-teal" />, label: 'Avg Score', value: `${avgScore}%` },
              {
      ...
      <footer className="border-t border-border bg-card py-3 px-4 text-center">
        <p className="text-xs text-muted-foreground font-body">
          © {new Date().getFullYear()} AIAPGET Unani PYQ &nbsp;·&nbsp; Built with{' '}
