...
import { ArrowLeft, CheckCircle2, Loader2, CreditCard, Building2, Wallet, Smartphone } from 'lucide-react';
...
const PAYMENT_METHODS: {
...
}[] = [
  {
    ...
    icon: <img src="/assets/generated/upi-badge.dim_120x48.png" alt="UPI" className="h-7 object-contain" />,
    badge: 'Recommended',
  },
  {
    id: 'credit',
    ...
    icon: <img src="/assets/generated/card-icon.dim_64x64.png" alt="Card" className="h-7 w-7 object-contain" />,
  },
  {
    id: 'debit',
    label: 'Debit Card',
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4">
        <div className="bg-card border border-border rounded-2xl p-8 max-w-sm w-full text-center shadow-lg space-y-4">
          ...
          <h2 className="text-xl font-heading font-bold text-foreground">Payment Successful!</h2>
          <p className="text-sm text-muted-foreground font-body">
            You're now subscribed to the <strong className="text-foreground">{planName}</strong>.
            Enjoy unlimited access to all AIAPGET Unani PYQ materials.
          </p>
          <div className="bg-muted rounded-xl p-3 text-sm font-body text-muted-foreground">
            <p>
              Amount paid: <strong className="text-foreground">{planPrice}</strong>
            </p>
            <p>
              Billing: <strong className="text-foreground capitalize">{planCycle}</strong>
            </p>
          </div>
          <Button
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-6 space-y-5">
        ...
        <div className="bg-card border border-gold/30 rounded-2xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground font-body capitalize">{planCycle} billing</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-heading font-bold text-gold">{planPrice}</p>
          </div>
        </div>
              ...
              >
                ...
                <div
                  className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${
                    selected === method.id ? 'border-gold' : 'border-muted-foreground'
                  }`}
                >
                  {selected === method.id && (
        ...
        <div className="bg-card border border-border rounded-xl p-4 space-y-3">
          ...
          {selected === 'wallet' && (
            <div className="space-y-2">
              <Label className="font-body text-sm">Select Wallet</Label>
              <div className="grid grid-cols-3 gap-2">
                {['Paytm', 'PhonePe', 'Amazon Pay', 'Google Pay', 'Mobikwik', 'Freecharge'].map(
                  (w) => (
                    <button
                      key={w}
